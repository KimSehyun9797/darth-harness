...
enable_live_status() {
  ...
  if [ "$input" = /dev/null ]; then
    jq -n --arg command "$new_command" \
      '{statusLine:{type:"command",command:$command,refreshInterval:60}}' > "$tmp"
  else
    jq --arg command "$new_command" '.statusLine = (.statusLine // {}) | .statusLine.command = $command' \
      "$input" > "$tmp"
  fi
  chmod 600 "$tmp"
  mv "$tmp" "$settings"
